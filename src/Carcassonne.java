...
public class Carcassonne {
    ...
    public int placeTiles(int r, int c){
        ...
        for (int i = 0; i < 4; i++) {
            if (isValid(r - 1, c) && !tileStack.isEmpty() && canPlaceTile(tileStack.get(0), r-1, c)){
                sum += placeTiles(r-1, c);
            }
            else if (isValid(r, c+1) && !tileStack.isEmpty() && canPlaceTile(tileStack.get(0), r,c+1)) {
                sum += placeTiles(r, c+ 1);
            }
            else if (isValid(r +1, c) && !tileStack.isEmpty() && canPlaceTile(tileStack.get(0), r+1, c)) {
                sum += placeTiles(r+1, c);
            }
            else if (isValid(r, c -1) && !tileStack.isEmpty() && canPlaceTile(tileStack.get(0), r, c -1)) {
                sum += placeTiles(r, c-1);
            }
            else if (!tileStack.isEmpty())
